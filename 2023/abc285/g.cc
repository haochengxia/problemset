...
void solve(){
		...
		rep(j,m){
			...
			if(i%2==j%2){
				...
				rep(k,4){
					int nx=i+dx[k];
					int ny=j+dy[k];
					if(0<=nx and 0<=ny and nx<n and ny<m and f[nx][ny] != '1'){
						mcc.add(id, nx*m+ny, 1, 0);
					}
				}
			}
			else{
				if(f[i][j]=='2'){
					mcc.add(id,t,1,-1); sum++;
				}
				else{
					mcc.add(id,t,1,0);
				}
	...
	mcc.add(t,s,100000,0);
	
	bool feasible = mcc.mincost_circulation();
 	if(feasible){
 		int ans = sum+mcc.circulation_cost();
 		o(!ans?"Yes":"No");
 	}
 	else o("No");
}
