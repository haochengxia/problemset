...
void solve(){
	cin>>n>>s>>q;
	rep(i,n-1) if(s[i]>s[i+1]) bad.insert(i);
	...
	while(q--){
		...
		if(t == 1){
			...
			a--;
			pos[s[a]-'a'].erase(a);
			if(a and s[a-1]>s[a]) bad.erase(a-1);
			if(a+1<n and s[a]>s[a+1]) bad.erase(a);
			s[a] = ss[0];
			pos[s[a]-'a'].insert(a);
			if(a and s[a-1]>s[a]) bad.insert(a-1);
			if(a+1<n and s[a]>s[a+1]) bad.insert(a);
		}
		else{
			int b;cin>>b;
			a--; b--;
			auto it = bad.lower_bound(a);
			if(it != bad.end() and (*it) < b){
				o("No");
			}
			else{
				int lb = s[a]-'a'+1;
