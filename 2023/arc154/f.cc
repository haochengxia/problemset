...
pair < poly, poly > prs (int l, int r) { // x / y
	if(l == r) {
		int i = l;
		int w = 1;
		int a = (ll) i * inv[n] % mod;
		int b = (ll) (mod - (n - i)) * inv[n] % mod;
		w = (ll) w * C(n - 1, i - 1) % mod;
		w = (ll) w * sgn(i - 1) % mod;
//		cout << w << " / " << a << '+' << b << "u" << endl;
		return make_pair(vi{w}, vi{a, b});
	}
	int mid = (l + r) >> 1;
	auto ls = prs(l, mid);
	auto rs = prs(mid + 1, r);
	return make_pair(ls.first * rs.second + rs.first * ls.second, 
		ls.second * rs.second);
}

int T[N];
pair < poly, poly > dc (int l, int r) { // x / y
	if(l == r) {
		return make_pair(vi{T[l]}, vi{1, mod - 1 - l});
	}
	int mid = (l + r) >> 1;
	auto ls = dc(l, mid);
	auto rs = dc(mid + 1, r);
...
int main () {
	...
	Pinit(max(n, m) * 2 + 4);
	auto qwq = prs(1, n);
	poly f = qwq.first * qwq.second.Rs(m + 1).Inv();
	f.rs(m + 1);
	poly g(m + 1);
	L(i, 0, m) 
	...
	f.Rev();
	L(i, 0, m) 
		g[i] = (ll) ifac[i] * sgn(i) % mod;
	g *= f, g.rs(m + 1), g.Rev();
	L(i, 0, m) 
		g[i] = (ll) g[i] * ifac[i] % mod, T[i] = g[i];
	auto u = dc(0, m);
	poly ns = u.first * u.second.Rs(m + 1).Inv();
	L(i, 1, m) 
		cout << ns[i] << '\n';
	return 0;
}